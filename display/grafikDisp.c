...
static void graphic_wait_ready(void)
{
    ...
    while(1) {
        graphic_ctrl_bit_set(B_E); /*sätt E positiv flank*/
        delay_500ns();
        c = GPIO_E.idr_high & LCD_BUSY; /* Vänta tills displayen inte är upptagen */

        graphic_ctrl_bit_clear(B_E); /*nollställ E negativ flank*/
        delay_500ns();
        if(c == 0)
            break; /*avbryt efter negativa flanken*/
    }
    delay_500ns(); 
...
void pixel(uint32_t x, uint32_t y, uint32_t set)
{
    ...
    graphic_writeCommand(LCD_ON, B_CS1 | B_CS2); /*tända skärm*/
}
